...
When investigating an incident for `buggy-demo-app`, always:

1. **Read the incident first** — understand what the human reporter observed
2. **Check pod phase** — `Running` with 0 restarts is the baseline; any other state is a problem
3. **Check Prometheus for error rates** — use the queries above; compare against the "normal" baseline
4. **Identify which endpoints are affected** — products (NPE), inventory (503), orders (latency) have distinct patterns
5. **Retrieve pod logs** — look for `NullPointerException` stacktraces; absence of errors with 503s is also diagnostic
6. **Distinguish intentional vs. unintentional faults** — the three known faults above are expected; anything else (OOMKill, CrashLoop, ImagePull errors) is a real incident
7. **Quantify impact** — always report error rates as percentages and latency as p50/p99 values from Prometheus
8. **Update the incident** — enrich it with your findings so the human team has all the data they need

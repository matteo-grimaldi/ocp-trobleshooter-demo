#!/usr/bin/env bash
# build-and-deploy.sh — Build and deploy the Prometheus MCP Server
set -euo pipefail
...
ENTRY_VALUE='{"url":"http://prometheus-mcp-server.coding-assistant.svc:8080/mcp","description":"PromQL queries against the in-cluster Thanos Querier. Instant and range queries with min/avg/max summaries."}'
...
if oc get configmap "${CATALOG_CM}" -n "${CATALOG_NS}" &>/dev/null; then
  ...
  oc patch configmap "${CATALOG_CM}" -n "${CATALOG_NS}" --type=merge \
    -p "{\"data\":{\"Prometheus\":${ESCAPED}}}"
else
  echo "  Creating ${CATALOG_CM}"
  oc create configmap "${CATALOG_CM}" -n "${CATALOG_NS}" \
    --from-literal="Prometheus=${ENTRY_VALUE}"
fi

echo ""
echo "=== Prometheus MCP server is live ==="
